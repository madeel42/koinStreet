...
import { API_URL_COINCAP, COINS_LIMIT } from './../../constants';
  ...
  data.map(item => {
    ...
    const maxSupply =
      item.maxSupply && !isNull(item.supply)
        ? parseFloat(item.maxSupply, 10)
        : supply;
    return {
      ...item,
      changePercent24Hr: !isNull(item.changePercent24Hr)
        ? parseFloat(item.changePercent24Hr, 10)
        : null,
      marketCapUsd: !isNull(item.marketCapUsd)
        ? parseFloat(item.marketCapUsd, 10)
        : null,
      maxSupply,
      priceUsd: !isNull(item.priceUsd) ? parseFloat(item.priceUsd, 10) : null,
      rank: !isNull(item.rank) ? parseInt(item.rank, 10) : null,
      supply,
      volumeUsd24Hr: !isNull(item.volumeUsd24Hr)
        ? parseFloat(item.volumeUsd24Hr, 10)
        : null,
...
const updateCoinByIdSelector = createSelector(
  [state => state.list, state => state.key, state => state.values],
  updateCoinById
);
