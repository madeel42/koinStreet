...
import { CHANGELLY_REF_ID } from './../constants';
...
const buy = (e, currency) => {
  ...
  window.open(
    `https://changelly.com/exchange/BTC/${currency}/1?ref_id=${CHANGELLY_REF_ID}`,
    '_blank'
  );
};
...
const sell = (e, currency) => {
  ...
  currency = currency !== 'BTC' ? currency : 'ETH';
  window.open(
    `https://changelly.com/exchange/${currency}/BTC/1?ref_id=${CHANGELLY_REF_ID}`,
    '_blank'
  );
};
